...
describe('config', ({ afterEach, test }) => {
    ...
    test('connect-fail', async () => {
        ...
        await sb.status()
            .then(() => Promise.reject(new Error('!throwAsExpected')), err => expect(err.code, 'ECONNREFUSED'));
    ...
    test('reconnect', async () => {
        ...
        await sb.request({ name: 'hello' }, { payload: 'Jack' }).then(() => Promise.reject(new Error("!throwAsExpected")), err => expect(err.message, 'No connection'));
    ...
    test('wait-endpoint', async () => {
        ...
        sb.waitEndpoint(req.header.from).then(err => queue.push(['endpoint-close', err]));
        await waitMillis(100);
        clientSb.shutdown();
...
describe("service", ({ beforeAll, afterAll, test }) => {
    ...
    test('streaming', async () => {
        ...
        expect(await rxjs.lastValueFrom(rxjs.fromEvent(res.payload, 'data').pipe(rxjs.takeUntil(rxjs.fromEvent(res.payload, 'end')), rxjs.buffer(rxjs.NEVER))), [
...
function makeQueue() {
    ...
    return {
        ...
        push(value) {
            items.push(value);
            while (items.length && waiters.length)
                waiters.shift()(items.shift());
        },
        take() {
